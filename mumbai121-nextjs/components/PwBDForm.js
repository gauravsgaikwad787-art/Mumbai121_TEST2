...
const railwayOptions = [
  'Central', 'Harbour', 'Western', 'Work From Home'
]
...
export default function PwBDForm() {
  ...
  function validate() {
    ...
    if (!formData.disability)               e.disability = 'Please select your disability type.'
  ...
  async function handleSubmit(e) {
    ...
    fd.append('disability', formData.disability)
    fd.append('email', formData.email)
    fd.append('whatsapp', formData.whatsapp)
    fd.append('railways', JSON.stringify(formData.railways))
    ...
    xhr.open('POST', `${BACKEND_RESUME}/register/pwbd`)
    xhr.send(fd)
  }
  ...
  if (submitted) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-600">Your PwBD registration has been recorded.</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      {/* Railway Lines */}
      <FormField label="Which railway line(s) would you prefer to work on?" required error={errors.railways}>
        {railwayOptions.map(line => (
          <label key={line} className="flex items-center gap-3 p-3 rounded-lg hover:bg-[#FFAC33]/10 cursor-pointer">
            ...
            <span className="text-gray-700 text-sm">{line === 'Work From Home' ? 'I would prefer to work from home' : line}</span>
          </label>
        ))}
      </FormField>
